...
const service = {
  ...
  chats: () => FIND_MANY_RESULT,
  chat: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Chat", () => {
  ...
  test("GET /chats/:id non existing", async () => {
    await request(app.getHttpServer())
  ...
  test("GET /chats/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/chats"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /chats existing resource", async () => {
